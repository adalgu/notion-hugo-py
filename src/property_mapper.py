...
THEME_PROPERTIES = {
    ...
    "subtitle": {
        "hugo_key": "subtitle",
        "description": "부제목 (일부 테마에서 지원)"
    },
    "linkTitle": {
        "hugo_key": "linkTitle",
        "description": "링크에 표시될 짧은 제목"
    },
    "layout": {
        "hugo_key": "layout",
        "description": "사용할 템플릿 레이아웃"
    }
}
...
class PropertyMapper:
    ...
    def should_skip_page(self, notion_properties):
        ...
        Returns:
            건너뛰기 여부 (Boolean)
        """
        # 'skipRendering' 속성이 있으면 그 값을 사용, 없으면 'doNotRendering' 속성을 확인 (하위 호환성)
        return notion_properties.get('skipRendering', notion_properties.get('doNotRendering', False)) == True
        ...
        for key, config in self.minimal_properties.items():
            ...
            if key not in notion_properties or not notion_properties[key]:
                ...
                if 'default' in config:
                    if isinstance(config['default'], str) and config['default'].startswith('<') and config['default'].endswith('>'):
