...
def convert_blocks_to_markdown(blocks: List[Dict[str, Any]]) -> str:
    ...
    for block in blocks:
        ...
        if block_type == 'paragraph':
            text = convert_rich_text_to_markdown(block.get('paragraph', {}).get('rich_text', []))
            if text:
                markdown += f"{text}\n\n"
            else:
                markdown += "\n"
            ...
            text = convert_rich_text_to_markdown(block.get('heading_1', {}).get('rich_text', []))
            markdown += f"# {text}\n\n"
        
        elif block_type == 'heading_2':
            text = convert_rich_text_to_markdown(block.get('heading_2', {}).get('rich_text', []))
            markdown += f"## {text}\n\n"
        
        elif block_type == 'heading_3':
            text = convert_rich_text_to_markdown(block.get('heading_3', {}).get('rich_text', []))
            markdown += f"### {text}\n\n"
        
        elif block_type == 'bulleted_list_item':
            text = convert_rich_text_to_markdown(block.get('bulleted_list_item', {}).get('rich_text', []))
            markdown += f"- {text}\n"
            list_type = 'bulleted'
        
        elif block_type == 'numbered_list_item':
            text = convert_rich_text_to_markdown(block.get('numbered_list_item', {}).get('rich_text', []))
            markdown += f"1. {text}\n"
            list_type = 'numbered'
        
        elif block_type == 'to_do':
            text = convert_rich_text_to_markdown(block.get('to_do', {}).get('rich_text', []))
            checked = block.get('to_do', {}).get('checked', False)
            checkbox = "[x]" if checked else "[ ]"
            markdown += f"{checkbox} {text}\n"
        
        elif block_type == 'toggle':
            text = convert_rich_text_to_markdown(block.get('toggle', {}).get('rich_text', []))
            ...
            text = convert_rich_text_to_markdown(block.get('code', {}).get('rich_text', []))
            language = block.get('code', {}).get('language', '')
            markdown += f"```{language}\n{text}\n```\n\n"
        
        elif block_type == 'quote':
            text = convert_rich_text_to_markdown(block.get('quote', {}).get('rich_text', []))
            markdown += f"> {text}\n\n"
