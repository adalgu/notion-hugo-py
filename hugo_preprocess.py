...
이 스크립트는 휴고 빌드 전에 실행되어 오류가 발생할 가능성이 있는 파일을 식별하고 
임시로 제외시켜 빌드 과정이 중단되지 않도록 합니다.

실행 방법:
    python hugo_preprocess.py [hugo 명령어 인자]

예:
    python hugo_preprocess.py server
    python hugo_preprocess.py --minify
"""
